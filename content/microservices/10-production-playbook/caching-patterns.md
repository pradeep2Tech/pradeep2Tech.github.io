...
```text
  ...
  SETNX lock:product:42 (TTL 5s)
        │
        Γö£── Lock acquired  → ONE thread queries DB → populates cache → DEL lock
        │
...
```text
  DB Write (WAL) ──Γû║ Debezium ──Γû║ Kafka ──Γû║ Eviction Worker ──Γû║ DEL redis:key
...
```text
  Cache invalidation hierarchy (best → acceptable):
    ...
    Γ£ù Never rely on cache update without eviction on mutation
```
