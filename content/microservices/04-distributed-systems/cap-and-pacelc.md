...
```text
  CP / PC (reject or block on partition):
    ΓÇó Financial ledger, billing, inventory holds
    ΓÇó Service registry (etcd), distributed locks
    ΓÇó "Better error than wrong balance"

  AP / EL (accept writes, serve stale reads):
    ΓÇó Product recommendations, social feeds
    ΓÇó Analytics counters, notification queues
    ΓÇó "Better stale feed than blank page"
```
