...
draft: false
description: "Batch-first micro-batch vs true streaming latency."
tags: ["technology-playbook", "comparison", "spark"]
categories: ["Technology Playbook"]
shortTitle: "Spark vs Flink"
module: 8
...
**Spark vs Flink** — Batch-first micro-batch vs true streaming latency. Use this comparison in architecture reviews and interviews to justify a choice with trade-offs, not slogans.
...
```mermaid
flowchart TD
  need[Integration / Data Need] --> eval{"Spark vs Flink"}
  eval --> pickA[Spark]
  eval --> pickB[Flink]
```
...
{{< comparison-table caption="Spark vs Flink" >}}
| Capability | Spark | Flink |
| :--- | :--- | :--- |
...
{{< interview-answer >}}
"There is no universal winner in **Spark vs Flink**. I compare delivery semantics, ops model, ordering, replay, and team skills. I give a concrete scenario — for example peak checkout events vs nightly batch — and map each option to that scenario before recommending one."
{{< /interview-answer >}}
