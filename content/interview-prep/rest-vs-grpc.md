...
draft: false
description: "HTTP/JSON ubiquity vs binary HTTP/2 performance and contracts."
tags: ["technology-playbook", "comparison", "rest"]
...
**REST vs gRPC** — HTTP/JSON ubiquity vs binary HTTP/2 performance and contracts. Use this comparison in architecture reviews and interviews to justify a choice with trade-offs, not slogans.
...
```mermaid
flowchart TD
  need[Integration / Data Need] --> eval{"REST vs gRPC"}
  eval --> pickA[Rest]
  eval --> pickB[Grpc]
```
...
{{< comparison-table caption="REST vs gRPC" >}}
...
{{< interview-answer >}}
"There is no universal winner in **REST vs gRPC**. I compare delivery semantics, ops model, ordering, replay, and team skills. I give a concrete scenario — for example peak checkout events vs nightly batch — and map each option to that scenario before recommending one."
{{< /interview-answer >}}
