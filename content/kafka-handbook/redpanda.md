...
draft: false
description: "Kafka-compatible streaming without ZooKeeper — simpler ops footprint."
tags: ["technology-playbook", "messaging", "redpanda"]
...
**Redpanda** — Kafka-compatible streaming without ZooKeeper — simpler ops footprint.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Redpanda)]
...
{{< interview-answer >}}
"I'd pick **Redpanda** when the domain needs kafka-compatible streaming without zookeeper. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
