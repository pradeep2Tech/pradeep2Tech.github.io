...
draft: false
description: "Managed queue — simple decoupling without broker operations."
tags: ["technology-playbook", "messaging", "sqs"]
...
**Amazon SQS** — Managed queue — simple decoupling without broker operations.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(SQS)]
...
{{< interview-answer >}}
"I'd pick **SQS** when the domain needs managed queue. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
