...
draft: false
description: "Managed pub/sub fan-out — push to queues, Lambda, and HTTP endpoints."
tags: ["technology-playbook", "messaging", "sns"]
...
**Amazon SNS** — Managed pub/sub fan-out — push to queues, Lambda, and HTTP endpoints.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(SNS)]
...
{{< interview-answer >}}
"I'd pick **SNS** when the domain needs managed pub/sub fan-out. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
