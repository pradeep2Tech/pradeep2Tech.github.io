...
draft: false
description: "Global async messaging — at-least-once delivery with ordering keys."
tags: ["technology-playbook", "messaging", "google"]
...
**Google Cloud Pub/Sub** — Global async messaging — at-least-once delivery with ordering keys.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Pub/Sub)]
...
{{< interview-answer >}}
"I'd pick **Pub/Sub** when the domain needs global async messaging. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
