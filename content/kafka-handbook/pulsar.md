...
draft: false
description: "Multi-tenant streaming — unified queue and log with geo-replication."
tags: ["technology-playbook", "messaging", "pulsar"]
...
**Apache Pulsar** — Multi-tenant streaming — unified queue and log with geo-replication.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Pulsar)]
...
{{< interview-answer >}}
"I'd pick **Pulsar** when the domain needs multi-tenant streaming. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
