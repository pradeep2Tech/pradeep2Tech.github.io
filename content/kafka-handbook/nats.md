...
draft: false
description: "Lightweight pub/sub — cloud-native, low latency, JetStream persistence."
tags: ["technology-playbook", "messaging", "nats"]
...
**NATS** — Lightweight pub/sub — cloud-native, low latency, JetStream persistence.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(NATS)]
...
{{< interview-answer >}}
"I'd pick **NATS** when the domain needs lightweight pub/sub. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
