...
The Memento Pattern **captures and externalizes an object's internal state** so it can be restored later, without violating encapsulation. The originator creates a memento; the caretaker stores it but cannot inspect or mutate internals. It powers undo stacks, checkpoints, game saves, and draft recovery — pairing naturally with [Command](/design-patterns/04-behavioral-patterns/command-pattern/) when undo needs full state snapshots.
...
| Inverse operations are cheap and well-defined | No | [Command](/design-patterns/04-behavioral-patterns/command-pattern/) `undo()` is lighter |
...
- **Pairs with:** [Command Pattern](/design-patterns/04-behavioral-patterns/command-pattern/), [Prototype Pattern](/design-patterns/02-creational-patterns/prototype-pattern/)
