...
draft: false
description: "Extensible API gateway — plugins for auth, rate limit, and transformation."
tags: ["technology-playbook", "cloud-native", "kong"]
...
**Kong Gateway** — Extensible API gateway — plugins for auth, rate limit, and transformation.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Kong)]
...
{{< interview-answer >}}
"I'd pick **Kong** when the domain needs extensible api gateway. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
