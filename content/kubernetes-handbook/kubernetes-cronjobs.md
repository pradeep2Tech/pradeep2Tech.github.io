...
draft: false
description: "Native cluster cron — containerized scheduled tasks."
tags: ["technology-playbook", "scheduler", "kubernetes"]
categories: ["Technology Playbook"]
shortTitle: "K8s CronJobs"
module: 5
...
**Kubernetes CronJobs** — Native cluster cron — containerized scheduled tasks.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(K8s CronJobs)]
...
{{< interview-answer >}}
"I'd pick **K8s CronJobs** when the domain needs native cluster cron. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
