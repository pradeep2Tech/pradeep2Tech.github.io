...
draft: false
description: "Elasticsearch + Logstash + Kibana — classic centralized logging."
tags: ["technology-playbook", "cloud-native", "elk"]
...
**ELK Stack** — Elasticsearch + Logstash + Kibana — classic centralized logging.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(ELK)]
...
{{< interview-answer >}}
"I'd pick **ELK** when the domain needs elasticsearch + logstash + kibana. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
