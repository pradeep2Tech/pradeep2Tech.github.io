...
draft: false
description: "Kubernetes ingress — HTTP routing into cluster services."
tags: ["technology-playbook", "cloud-native", "nginx"]
categories: ["Technology Playbook"]
shortTitle: "NGINX Ingress"
module: 6
...
**NGINX Ingress Controller** — Kubernetes ingress — HTTP routing into cluster services.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(NGINX Ingress)]
...
{{< interview-answer >}}
"I'd pick **NGINX Ingress** when the domain needs kubernetes ingress. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
