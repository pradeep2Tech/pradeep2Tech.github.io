...
draft: false
description: "L4/L7 load balancer — high throughput TCP and HTTP routing."
tags: ["technology-playbook", "cloud-native", "haproxy"]
...
**HAProxy** — L4/L7 load balancer — high throughput TCP and HTTP routing.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(HAProxy)]
...
{{< interview-answer >}}
"I'd pick **HAProxy** when the domain needs l4/l7 load balancer. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
