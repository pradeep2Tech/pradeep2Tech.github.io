...
draft: false
description: "Lightweight service mesh — minimal overhead, Rust data plane."
tags: ["technology-playbook", "cloud-native", "linkerd"]
...
**Linkerd** — Lightweight service mesh — minimal overhead, Rust data plane.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Linkerd)]
...
{{< interview-answer >}}
"I'd pick **Linkerd** when the domain needs lightweight service mesh. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
