...
draft: false
description: "Cloud-native edge router — auto-discovery with Kubernetes and Let's Encrypt."
...
**Traefik** — Cloud-native edge router — auto-discovery with Kubernetes and Let's Encrypt.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Traefik)]
...
{{< interview-answer >}}
"I'd pick **Traefik** when the domain needs cloud-native edge router. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
