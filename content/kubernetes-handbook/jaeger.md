...
draft: false
description: "Distributed tracing — request flow across microservices."
tags: ["technology-playbook", "cloud-native", "jaeger"]
...
**Jaeger** — Distributed tracing — request flow across microservices.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Jaeger)]
...
{{< interview-answer >}}
"I'd pick **Jaeger** when the domain needs distributed tracing. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
