...
draft: false
description: "Vendor-neutral telemetry SDK — traces, metrics, logs export."
tags: ["technology-playbook", "cloud-native", "opentelemetry"]
...
**OpenTelemetry** — Vendor-neutral telemetry SDK — traces, metrics, logs export.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(OpenTelemetry)]
...
{{< interview-answer >}}
"I'd pick **OpenTelemetry** when the domain needs vendor-neutral telemetry sdk. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
