...
draft: false
description: "Managed REST/WebSocket/API — Lambda and VPC integration."
tags: ["technology-playbook", "cloud-native", "aws"]
categories: ["Technology Playbook"]
shortTitle: "AWS API Gateway"
module: 6
...
**AWS API Gateway** — Managed REST/WebSocket/API — Lambda and VPC integration.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(AWS API Gateway)]
...
{{< interview-answer >}}
"I'd pick **AWS API Gateway** when the domain needs managed rest/websocket/api. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
