...
draft: false
description: "Log aggregation like Prometheus — label-based, cost-efficient storage."
tags: ["technology-playbook", "cloud-native", "loki"]
...
**Grafana Loki** — Log aggregation like Prometheus — label-based, cost-efficient storage.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Loki)]
...
{{< interview-answer >}}
"I'd pick **Loki** when the domain needs log aggregation like prometheus. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
