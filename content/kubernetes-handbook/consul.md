...
draft: false
description: "Service mesh and discovery — multi-platform connectivity."
tags: ["technology-playbook", "cloud-native", "consul"]
...
**HashiCorp Consul** — Service mesh and discovery — multi-platform connectivity.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Consul)]
...
{{< interview-answer >}}
"I'd pick **Consul** when the domain needs service mesh and discovery. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
