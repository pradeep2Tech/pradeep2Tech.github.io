...
draft: false
description: "Rich data structures vs pure cache — persistence and pub/sub."
tags: ["technology-playbook", "comparison", "redis"]
...
**Redis vs Memcached** — Rich data structures vs pure cache — persistence and pub/sub. Use this comparison in architecture reviews and interviews to justify a choice with trade-offs, not slogans.
...
```mermaid
flowchart TD
  need[Integration / Data Need] --> eval{"Redis vs Memcached"}
  eval --> pickA[Redis]
  eval --> pickB[Memcached]
```

---

## 4. When to Choose Redis
...
{{< comparison-table caption="Redis vs Memcached" >}}
...
{{< interview-answer >}}
"There is no universal winner in **Redis vs Memcached**. I compare delivery semantics, ops model, ordering, replay, and team skills. I give a concrete scenario — for example peak checkout events vs nightly batch — and map each option to that scenario before recommending one."
{{< /interview-answer >}}
