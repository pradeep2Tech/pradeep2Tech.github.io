...
draft: false
description: "Write-optimized wide rows vs flexible documents."
tags: ["technology-playbook", "comparison", "cassandra"]
categories: ["Technology Playbook"]
shortTitle: "Cassandra vs MongoDB"
module: 8
...
**Cassandra vs MongoDB** — Write-optimized wide rows vs flexible documents. Use this comparison in architecture reviews and interviews to justify a choice with trade-offs, not slogans.
...
```mermaid
flowchart TD
  need[Integration / Data Need] --> eval{"Cassandra vs MongoDB"}
  eval --> pickA[Cassandra]
  eval --> pickB[Mongodb]
```

---

## 4. When to Choose Cassandra
...
{{< comparison-table caption="Cassandra vs MongoDB" >}}
| Capability | Cassandra | Mongodb |
| :--- | :--- | :--- |
...
{{< interview-answer >}}
"There is no universal winner in **Cassandra vs MongoDB**. I compare delivery semantics, ops model, ordering, replay, and team skills. I give a concrete scenario — for example peak checkout events vs nightly batch — and map each option to that scenario before recommending one."
{{< /interview-answer >}}
