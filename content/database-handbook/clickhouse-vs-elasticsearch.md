...
draft: false
description: "OLAP columnar analytics vs inverted-index search and logs."
tags: ["technology-playbook", "comparison", "clickhouse"]
categories: ["Technology Playbook"]
shortTitle: "ClickHouse vs ES"
module: 8
...
**ClickHouse vs Elasticsearch** — OLAP columnar analytics vs inverted-index search and logs. Use this comparison in architecture reviews and interviews to justify a choice with trade-offs, not slogans.
...
```mermaid
flowchart TD
  need[Integration / Data Need] --> eval{"ClickHouse vs ES"}
  eval --> pickA[Clickhouse]
  eval --> pickB[Elasticsearch]
```

---

## 4. When to Choose Clickhouse
...
{{< comparison-table caption="ClickHouse vs Elasticsearch" >}}
| Capability | Clickhouse | Elasticsearch |
| :--- | :--- | :--- |
...
{{< interview-answer >}}
"There is no universal winner in **ClickHouse vs ES**. I compare delivery semantics, ops model, ordering, replay, and team skills. I give a concrete scenario — for example peak checkout events vs nightly batch — and map each option to that scenario before recommending one."
{{< /interview-answer >}}
