...
This is the same partial-index pattern used in [Outbox/Inbox Performance Tuning](/database-handbook/outbox-inbox-performance-tuning/) (`WHERE sent = 0`) — restrict the B+ Tree to only the rows your hot-path queries touch.
...
Pair time-ordered [primary keys](/database-handbook/primary-key-selection-strategies/) with these primitives before tackling [zero-downtime migrations](/database-handbook/zero-downtime-migration-frameworks/) on high-traffic tables — the index and view layer you build now determines how painful expand-and-contract becomes later.
