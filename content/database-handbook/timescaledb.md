...
draft: false
description: "PostgreSQL extension for time-series — SQL familiarity plus hypertables."
tags: ["technology-playbook", "database", "timescaledb"]
categories: ["Technology Playbook"]
shortTitle: "TimescaleDB"
module: 3
...
**TimescaleDB** is a **time series** store. PostgreSQL extension for time-series — SQL familiarity plus hypertables.

---

## 2. What Problem It Solves

| Scenario | Why TimescaleDB |
| :--- | :--- |
| Primary application datastore | Metrics, IoT telemetry, downsampling, and retention policies |
...
- Access patterns align with **time-series** strengths (metrics, iot telemetry, downsampling, and retention policies)
...
| :--- | :--- |
| **Same class** | See [Databases module](/technology-playbook/) for peers in time-series |
...
{{< comparison-table >}}
| Dimension | TimescaleDB strength | Watch out for |
| :--- | :--- | :--- |
| **Data model** | Optimized for time-series access paths | Misfit patterns cause performance cliffs |
...
{{< interview-answer >}}
"For **TimescaleDB**, I'd choose it when workloads need metrics, iot telemetry, downsampling, and retention policies and the team can operate it responsibly. I'd compare it against other **time-series** options on ops burden, licensing, and cloud managed offerings. I'd also state what I would **not** store there — for example heavy analytics on OLTP or graph traversals on plain relational without extension."
{{< /interview-answer >}}
