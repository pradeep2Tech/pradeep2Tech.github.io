...
**Trade-off:** This method introduces minor polling latencies and creates continuous read/write overhead on the primary database engine. Operational tuning for polling stress is covered in [Outbox/Inbox Performance Tuning](/database-internals/outbox-inbox-performance-tuning/).
...
Downstream consumers that receive at-least-once delivery must pair with the [Transactional Inbox pattern](/database-internals/transactional-inbox-pattern/) for idempotent ingestion.
