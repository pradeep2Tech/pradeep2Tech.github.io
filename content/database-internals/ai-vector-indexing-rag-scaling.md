...
Under [eventual consistency](/database-internals/distributed-consistency-primitives/), replica lag means a freshly embedded document may not appear in vector search results on read replicas until the ANN index catches up — a transient window RAG pipelines must account for in freshness SLAs.
...
1. **The Sparse Path:** Evaluates standard lexical keyword matches over an inverted index structure using BM25 statistics — the same inverted-index principle as [GIN JSONB indexing](/database-internals/advanced-schema-optimization/), applied to full-text token maps.
...
| :--- | :--- | :--- |
| **Replica lag / stale reads** | CAP / PACELC trade-offs | [Distributed Consistency](/database-internals/distributed-consistency-primitives/) |
