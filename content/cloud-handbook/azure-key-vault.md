...
draft: false
description: "Secrets, keys, and certificates for Azure workloads."
tags: ["technology-playbook", "cloud-native", "azure"]
categories: ["Technology Playbook"]
shortTitle: "Key Vault"
module: 6
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Key Vault)]
...
{{< interview-answer >}}
"I'd pick **Key Vault** when the domain needs reliable async integration. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
