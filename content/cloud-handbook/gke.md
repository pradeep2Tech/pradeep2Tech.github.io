...
draft: false
description: "Managed Kubernetes on GCP — autopilot mode and strong networking."
tags: ["technology-playbook", "cloud-native", "gke"]
...
**Google GKE** — Managed Kubernetes on GCP — autopilot mode and strong networking.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(GKE)]
...
{{< interview-answer >}}
"I'd pick **GKE** when the domain needs managed kubernetes on gcp. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
