...
draft: false
description: "Managed Kubernetes on Azure — Azure AD integration and node pools."
tags: ["technology-playbook", "cloud-native", "aks"]
...
**Azure AKS** — Managed Kubernetes on Azure — Azure AD integration and node pools.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(AKS)]
...
{{< interview-answer >}}
"I'd pick **AKS** when the domain needs managed kubernetes on azure. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
