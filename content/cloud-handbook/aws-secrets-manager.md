...
draft: false
description: "Managed rotation for DB and API credentials on AWS."
tags: ["technology-playbook", "cloud-native", "aws"]
categories: ["Technology Playbook"]
shortTitle: "Secrets Manager"
module: 6
...
**AWS Secrets Manager** — Managed rotation for DB and API credentials on AWS.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Secrets Manager)]
...
{{< interview-answer >}}
"I'd pick **Secrets Manager** when the domain needs reliable async integration. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
