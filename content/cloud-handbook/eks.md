...
draft: false
description: "Managed Kubernetes on AWS — control plane ops handled by AWS."
tags: ["technology-playbook", "cloud-native", "eks"]
...
**Amazon EKS** — Managed Kubernetes on AWS — control plane ops handled by AWS.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(EKS)]
...
{{< interview-answer >}}
"I'd pick **EKS** when the domain needs managed kubernetes on aws. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
