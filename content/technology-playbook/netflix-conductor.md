...
draft: false
description: "Microservice orchestration — JSON-defined workflows over REST workers."
tags: ["technology-playbook", "workflow", "netflix"]
...
**Netflix Conductor** — Microservice orchestration — JSON-defined workflows over REST workers.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Conductor)]
...
{{< interview-answer >}}
"I'd pick **Conductor** when the domain needs microservice orchestration. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
