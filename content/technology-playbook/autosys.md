...
draft: false
description: "CA workload scheduler — mainframe and distributed job chains in large enterprises."
tags: ["technology-playbook", "scheduler", "autosys"]
...
**AutoSys** — CA workload scheduler — mainframe and distributed job chains in large enterprises.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(AutoSys)]
...
{{< interview-answer >}}
"I'd pick **AutoSys** when the domain needs ca workload scheduler. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
