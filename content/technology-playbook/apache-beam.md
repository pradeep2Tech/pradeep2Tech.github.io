...
draft: false
description: "Portable pipeline API — run the same logic on Spark, Flink, or Dataflow."
...
**Apache Beam** — Portable pipeline API — run the same logic on Spark, Flink, or Dataflow.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Beam)]
...
{{< interview-answer >}}
"I'd pick **Beam** when the domain needs portable pipeline api. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
