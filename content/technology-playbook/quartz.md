...
draft: false
description: "Java cron scheduler — clustered jobs inside application JVM."
tags: ["technology-playbook", "scheduler", "quartz"]
...
**Quartz Scheduler** — Java cron scheduler — clustered jobs inside application JVM.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Quartz)]
...
{{< interview-answer >}}
"I'd pick **Quartz** when the domain needs java cron scheduler. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
