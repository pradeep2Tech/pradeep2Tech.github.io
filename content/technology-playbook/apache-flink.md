...
draft: false
description: "Stream-first processing — low-latency stateful analytics."
...
**Apache Flink** — Stream-first processing — low-latency stateful analytics.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Flink)]
...
{{< interview-answer >}}
"I'd pick **Flink** when the domain needs stream-first processing. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
