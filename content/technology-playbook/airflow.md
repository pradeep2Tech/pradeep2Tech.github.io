...
draft: false
description: "DAG-based data pipeline scheduler — batch ETL and ML prep."
tags: ["technology-playbook", "workflow", "airflow"]
...
**Apache Airflow** — DAG-based data pipeline scheduler — batch ETL and ML prep.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Airflow)]
...
{{< interview-answer >}}
"I'd pick **Airflow** when the domain needs dag-based data pipeline scheduler. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
