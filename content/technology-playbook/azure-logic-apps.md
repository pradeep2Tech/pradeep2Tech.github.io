...
draft: false
description: "Low-code integration and rules — connectors and visual workflows."
tags: ["technology-playbook", "rules", "azure"]
categories: ["Technology Playbook"]
shortTitle: "Logic Apps"
module: 5
...
**Azure Logic Apps** — Low-code integration and rules — connectors and visual workflows.
...
```mermaid
flowchart LR
  producer[Order Service] --> broker[(Logic Apps)]
...
{{< interview-answer >}}
"I'd pick **Logic Apps** when the domain needs low-code integration and rules. I clarify delivery guarantees, ordering needs, and ops model — managed cloud queue vs self-hosted Kafka. I always mention idempotent consumers and dead-letter handling."
{{< /interview-answer >}}
